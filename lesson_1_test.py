...
def test_form_elements():
    ...
    sleep(1)
    driver.find_element(By.CSS_SELECTOR, 'input[name="zip-code"]').clear()  # Убедитесь, что почтовый индекс пуст, чтобы имитировать пропущенный ввод
    sleep(1)
    ...
    # Убедитесь, что все остальные поля не помечены как недопустимые (допустимый цвет по умолчанию - 'rgba(209, 231, 221, 1)')
...
# Чтобы запустить тест с помощью pytest, выполните в терминале следующую команду:
# python -m pytest
